...
template <class Hash, bool is_little_endian, class word_type, size_t state_cnt,
          size_t block_size>
struct length_extendable_hash {
  ...
  length_extendable_hash(const std::array<word_type, state_cnt> &state)
      : state(state) {}
  length_extendable_hash(size_t secret_len, py::bytes py_pt_cur,
                         py::bytes py_digest_init)
  ...
  static void string_to_array(const std::string &s, auto &arr, size_t cnt) {
    ...
    if constexpr (is_little_endian)
      for (size_t i = 0; i < bytes_per_word * cnt; ++i)
        arr[i / bytes_per_word] |= (word_type)(unsigned char)s[i]
                                   << i % bytes_per_word * 8;
    else
      for (size_t i = 0; i < bytes_per_word * cnt; ++i)
        arr[i / bytes_per_word] |=
            (word_type)(unsigned char)s[i]
            << (bytes_per_word - 1 - i % bytes_per_word) * 8;
  }
  static void pad(std::string &pt, size_t original_len) {
    pt.push_back(0x80);
    for (size_t len = original_len + 1;
         len % block_size != block_size - 2 * bytes_per_word; ++len)
      pt.push_back(0x00);
    original_len *= 8;
...
struct MD5_impl : length_extendable_hash<MD5_impl, true, uint32_t, 4, 64> {
  ...
  static void compress(const std::string &chunk,
...
struct SHA256_impl
    : length_extendable_hash<SHA256_impl, false, uint32_t, 8, 64> {
...
// Source: https://en.wikipedia.org/wiki/SHA-2#Pseudocode
struct SHA512_impl
    : length_extendable_hash<SHA512_impl, false, uint64_t, 8, 128> {
  ...
  // primes 2..19
  SHA512_impl()
      : length_extendable_hash({0x6a09e667f3bcc908, 0xbb67ae8584caa73b,

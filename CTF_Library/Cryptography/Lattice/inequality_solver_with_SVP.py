    ...
    def solve(self, repeat=1):
        ...
        for iteration in range(repeat):
            ...
            for i in range(m):
                ...
                mat = mat.stack(
                    vector(
                        ZZ, [self.mods[i] if n + i == j else 0 for j in range(n + m)]
                    )
                )
            mat *= 2
            ...
            mat = mat.stack(
                vector(
                    ZZ,
                    [-(low[i] + high[i] + offset[i]) // 2 for i in range(n + m)] + [1],
                )
            )
            ...
            if iteration == 0:
                if nr >= 300:
                    print(
                        f"[INFO] <inequality_solver_with_SVP> Aborting Gaussian heurstic: matrix too big"
                    )
                else:
                    det = (mat * mat.T).determinant()
                    if det == 0:
                        print(
                            f"[ERROR] <inequality_solver_with_SVP> linearly dependent row vectors"
                        )
                        assert False
                    det = isqrt(det)
                    cnt = 1
                    for l, h in zip(low, high):
                        cnt *= h - l + 1
                    print(
                        f"[INFO] <inequality_solver_with_SVP> Expected number of solutions: {int(round(cnt / det)) + 1}"
                    )
            ...
            for row in mat:
                if not 0 <= row[-1] <= 1:
                    continue
                assigned_value = [
                    int(row[i] + (low[i] + high[i] + offset[i]) // 2 * row[-1]) // 2
                    for i in range(n)
                ]
                equation_value = [
                    int(row[i] + (low[i] + high[i] + offset[i]) // 2 * row[-1]) // 2
                ...
                if any(
                    not self.lows[i] <= equation_value[i] <= self.highs[i]
                    for i in range(m)
                ):
                    continue
                ...
                if any(
                    not self.var_low[i] <= assigned_value[i] <= self.var_high[i]
                    for i in range(n)
                ):
                    print(
                        f"[INFO] <inequality_solver_with_SVP> assignment out of range"
                    )
                    print()
                    continue

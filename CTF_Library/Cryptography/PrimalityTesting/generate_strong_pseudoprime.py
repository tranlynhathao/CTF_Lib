...
def generate_strong_pseudoprime_2(bases: list, min_bit_length: int, count: int, check):
    ...
    while len(res) < count:
        p, q = p + mod, q + 2 * mod
        if all(
            miller(p * q, b) if b in (3, 5) else miller_light(p, q, b)
            for b in map(Zmod(p * q), reversed(bases))
        ) and check(p * q, p, q):
            res.append((p * q, p, q))
    return res
...
def generate_strong_pseudoprime_3(
    bases: list, min_bit_length: int, count: int, is_valid: None
):
        ...
        if z and m:
            ...
            while len(res) < count:
                ...
                if (
                    ...
                    and is_prime(p2)
                    and is_prime(p3)
                    and is_valid(p1 * p2 * p3, p1, p2, p3)
                ):

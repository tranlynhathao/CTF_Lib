...
def ECDDHP_prime_power_mod(
    ...
    from CTF_Library.Cryptography.EllipticCurve.EC_coordinate_normalizer import (
        EC_coordinate_normalizer,
    )
    from CTF_Library.Cryptography.EllipticCurve.ECDLP_prime_power_mod import (
        ECDLP_prime_power_mod,
    )
    from CTF_Library.Cryptography.EllipticCurve.custom_elliptic_curve import (
        custom_elliptic_curve,
    )
    ...
    targets = [
        (tuple(map(int, normalizer.map(*R))), tuple(map(int, normalizer.map(*S))))
        for R, S in targets
    ]
    ...
    if desc == 0:
        ...
        k = ECDLP_prime_power_mod(
            p, 1, coef, (Px, Py), (Qx, Qy), threshold, threshold2
        )[0]
        ...
        if QF.weil_pairing(PF, PF.order()) != 1:
            print(
                f"[WARNING]<ECDDHP_prime_power_mod>, P and Q must be linearly dependent"
            )
            return None
        res = []
        if ECF.order() == p + 1 and (coef[0] % p == 0 or coef[1] % p == 0):
            ...
            for R, S in targets:
                RF, SF = ECF(*R), ECF(*S)
                if ECF2(PF).weil_pairing(distortion_map(SF), p + 1) != ECF2(
                    QF
                ).weil_pairing(distortion_map(RF), p + 1):
                    res.append(0)
                elif e == 1:
                    res.append(1)
                else:
            ...
            for i, (R, S) in enumerate(targets):
                RF, SF = ECF(*R), ECF(*S)
                if SF.weil_pairing(RF, RF.order()) != 1 or SF.weil_pairing(
                    PF, PF.order()
                ) != RF.weil_pairing(QF, QF.order()):
                    res[i] = 0
            k = ECDLP_prime_power_mod(
                p, e, coef, (Px, Py), (Qx, Qy), threshold, threshold2
            )[0]
...
if __name__ == "__main__":
    ...
    def test_singular(ans):
        ...
        assert [ans] == ECDDHP_prime_power_mod(
            p, e, coef, P.xy(), Q.xy(), [(R.xy(), S.xy())]
        )
    ...
    def test_low_embedding_degree(ans):
        print(
            f"[INFO]<ECDDHP_prime_power_mod> test_low_embedding_degree begin with {ans = }"
        )
    ...
    def test_hypersingular_qudratic_twist_cubic(ans):
        print(
            f"[INFO]<ECDDHP_prime_power_mod> test_hypersingular_qudratic_twist_cubic begin with {ans = }"
        )
        p, e = 2717597692908121319788497985451, 4
        coef = [
            ,
        ...
        print(
            f"[INFO]<ECDDHP_prime_power_mod> test_hypersingular_qudratic_twist_cubic end\n"
        )

    def test_hypersingular_qudratic_twist_quadratic(ans):
        print(
            f"[INFO]<ECDDHP_prime_power_mod> test_hypersingular_qudratic_twist_quadratic begin with {ans = }"
        )
        ...
        print(
            f"[INFO]<ECDDHP_prime_power_mod> test_hypersingular_qudratic_twist_quadratic end\n"
        )

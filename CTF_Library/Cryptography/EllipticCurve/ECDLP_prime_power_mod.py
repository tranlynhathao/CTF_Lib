    ...
    if desc == 0:
        ...
        def update_with_singular_curve_attack():
            if threshold == -1 or e != 1:  # I don't know how to solve prime power case
                return 0, 1
            print(
                f"[INFO]<ECDLP_prime_power_mod> update_with_singular_curve_attack begin"
            )
            x = F["X"].gen()
            ...
            if len(roots) == 1:
                ...
                for q, f in factor(u_order):
                    if q >= threshold2:
                        print(
                            f"[INFO]<ECDLP_prime_power_mod> update_with_singular_curve_attack ignoring factor {q}^{f}"
                        )
                        large_factors *= q**f
                return (v**large_factors).log(
                    u**large_factors
                ), u_order // large_factors
            else:
                assert False
            print(
                f"[INFO]<ECDLP_prime_power_mod> update_with_singular_curve_attack end"
            )
        ...
        if QF.weil_pairing(PF, p_order) != 1:
            print(
                f"[WARNING]<ECDLP_prime_power_mod>, P and Q must be linearly dependent"
            )
            return None, None
        ...
        def update_with_small_factor():
            ...
            for q, f in factor(p_order):
                if q >= threshold:
                    print(
                        f"[INFO]<ECDLP_prime_power_mod> update_with_small_factor ignoring factor {q}^{f}"
                    )
                    large_factors *= q**f
        ...
        def update_with_MOV_attack():
            ...
            for d in range(1, 7):
                ...
                return 0, 1
            print(
                f"[INFO]<ECDLP_prime_power_mod> update_with_MOV_attack begin with embedding degree {d}"
            )
            large_factors = 1
            for q, f in list(factor(p - 1)) + list(factor((p**d - 1) / (p - 1))):
                if q >= threshold2:
                    print(
                        f"[INFO]<ECDLP_prime_power_mod> update_with_MOV_attack ignoring factor {q}^{f}"
                    )
                    large_factors *= q**f
            ...
            k = beta.log(alpha)
            print(
                f"[INFO]<ECDLP_prime_power_mod> update_with_MOV_attack discrete log took {time.time() - start_time} seconds"
            )
        ...
        attack_list += [
            update_with_prime_power,
            update_with_small_factor,
            update_with_Smart_attack,
            update_with_MOV_attack,
        ]
    r, m = 0, 1
...
if __name__ == "__main__":
    ...
    def test_singular():
        ...
        p, e = (
            229054522729978652250851640754582529779,
            1,
        )  # not sure how to deal with high e for singular curve
        coef = [-75, -250]
    ...
    def test_power():
        ...
        P = EC(
            201395103510950985196528886887600944697931024970644444173327129750000389064102542826357168547230875812115987973230106228243893553395960867041978131850021580112077013996963515239128729448812815223970675917812499157323530103467271226,
    ...
    def test_anomalous():
        ...
        p, e = (
            ...
            1,
        )  # not sure how to deal with high e for anomalous curve
        coef = [

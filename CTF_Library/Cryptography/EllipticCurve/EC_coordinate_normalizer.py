...
class EC_coordinate_normalizer:
    ...
    def map(self, x, y):
        ...
        assert (
            y**2 + self.init_coef[0] * x * y + self.init_coef[2] * y
            == x**3
            + self.init_coef[1] * x**2
            + self.init_coef[3] * x
            + self.init_coef[4]
        ), f"[ERROR]<EC_coordinate_normalizer> the point is not on the curve"
        y += (self.init_coef[0] * x + self.init_coef[2]) / self.R(2)
        x += (self.init_coef[1] + self.init_coef[0] ** 2 / self.R(4)) / self.R(3)
        assert y**2 == x**3 + self.coef[0] * x + self.coef[1], (
            f"[ERROR]<EC_coordinate_normalizer> there's something wrong with the implementation"
        )
        return x, y

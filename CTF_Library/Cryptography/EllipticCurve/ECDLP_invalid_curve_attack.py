...
def ECDLP_invalid_curve_attack(
    p: int,
    a: int,
    multiply_by_secret,
    required_modulus_size: int,
    curve_count: int = 1,
    threshold: int = 2**40,
):
    import math
    ...
    for b in map(F, range(1, p)):
        ...
        print(
            f"[ERROR]<ECDLP_invalid_curve_attack_single_coordinate> fail to generate enough curves with modulus equal or greater than {required_modulus_size}"
        )
        assert False
    return r, m

...
def generate_weierstrass_p_from_g2_and_g3(
    g2, g3, precision: int = 200, derivative: int = 0
):
    import mpmath
...
def generate_weierstrass_p_from_w1_and_w2(
    w1, w2, precision: int = 200, derivative: int = 0
):
    from mpmath import jtheta, pi, exp, sqrt, polyroots, agm, log
...
def generate_weierstrass_p_from_elliptic_curve(
    a, b, precision: int = 200, derivative: int = 0
):
...
if __name__ == "__main__":
    ...
    def test_wp_from_g_value(g2, g3, derivative, z, expected):
        assert is_close(
            generate_weierstrass_p_from_g2_and_g3(g2, g3, derivative=derivative)(z),
            expected,
        )

    def test_wp_from_g_differential_equation(g2, g3, z):
        wp = [
            generate_weierstrass_p_from_g2_and_g3(g2, g3, derivative=d)(z)
            for d in range(6)
        ]
    ...
    for args in [(5 + 3j, 2 + 7j, 1 + 1j)]:
        test_wp_from_g_differential_equation(*args)
    for args in [(1 + 4j, 1 + 15j, 1 + 1j, 0.0076936368424553 - 0.498821838483149j)]:
        test_wp_from_w_value(*args)
    for args in [(1 + 4j, 1 + 1j, -0.430565782630798 - 3.62705469588323e-16j)]:

class _custom_elliptic_curve_point:
    ...
    def __neg__(self):
        if self.is_origin():
            return self
        return _custom_elliptic_curve_point(
            self.p,
            self.params,
            self._x,
            (-self.params[0] * self._x - self.params[2] - self._y) % self.p,
        )
    ...
    def _double(self):
        ...
        lam = (
            (
                3 * self._x**2
                + 2 * self.params[1] * self._x
                - self.params[0] * self._y
                + self.params[3]
            )
            * pow(2 * self._y + self.params[0] * self._x + self.params[2], -1, self.p)
            % self.p
        )
        x3 = (lam**2 + self.params[0] * lam - self.params[1] - 2 * self._x) % self.p
        y3 = (
            -self.params[0] * x3 - self.params[2] - lam * x3 + lam * self._x - self._y
        ) % self.p
    ...
    def __add__(self, other):
        ...
        x3 = (
            lam**2 + self.params[0] * lam - self.params[1] - self._x - other._x
        ) % self.p
        y3 = (
            -self.params[0] * x3 - self.params[2] - lam * x3 + lam * self._x - self._y
        ) % self.p
...
class custom_elliptic_curve:
    ...
    def _on_curve(self, x, y):
        return (y**2 + self.params[0] * x * y + self.params[2] * y) % self.p == (
            x**3 + self.params[1] * x**2 + self.params[3] * x + self.params[4]
        ) % self.p

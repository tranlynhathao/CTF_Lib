...
def ECDLP_invalid_curve_attack_single_coordinate(
    p: int,
    a: int,
    b: int,
    multiply_by_secret,
    use_initial_curve=True,
    threshold: int = 2**40,
    threshold2: int = 2**50,
):
    import math
    ...
    def process_for_twist(t):
        ...
        print(
            f"[INFO]<ECDLP_invalid_curve_attack_single_coordinate> Curve with twist {t} is of order {EC.order()} with factorization {factor(EC.order())}"
        )
        p = EC.gen(0)
        cand_sp = EC.lift_x(F(multiply_by_secret(p.x() / t)) * t)
        for sp in [cand_sp, -cand_sp]:
            print(
                f"[INFO]<ECDLP_invalid_curve_attack_single_coordinate> solving ECDLP with {sp} over the base {p}"
            )
            r, m = ECDLP_prime_power_mod(
                p, 1, [a, b], p.xy(), sp.xy(), threshold, threshold2
            )
            rem.append(r)

...
def forge_CBC_ciphertext_with_padding_oracle(
    plaintext: bytes, padding_oracle, hint=None, faulty=False, pool=list(range(2**8))
):
    ...
    if hint is not None:
        hm, hiv, hct = hint
        assert (
            isinstance(hm, bytes) and isinstance(hiv, bytes) and isinstance(hct, bytes)
        )
    ...
    for i in reversed(range(n // pad_len - int(hint != None))):
        for j in reversed(range(pad_len)):
            print(
                f"[INFO] <forge_CBC_ciphertext_with_padding_oracle> Forging block {i} at index {j}"
            )
            for k in range(j, pad_len):
                plaintext[pad_len * i + k] ^= pad_len - j ^ (
                    pad_len - j - 1 if k > j else 0
                )
            q_next = []
            for ciphertext in q:
                print(
                    f"[INFO] <forge_CBC_ciphertext_with_padding_oracle> {ciphertext = }"
                )
                candidate = []
                for x in pool:
                    ciphertext[pad_len * i + j] = x
                    cur_ciphertext = (
                        ciphertext[: pad_len * i]
                        + list(
                            x ^ y
                            for x, y in zip(
                                plaintext[pad_len * i : pad_len * (i + 1)],
                                ciphertext[pad_len * i : pad_len * (i + 1)],
                            )
                        )
                        + ciphertext[pad_len * (i + 1) : pad_len * (i + 2)]
                    )
                    resp = padding_oracle(
                        bytes(cur_ciphertext[:pad_len]), bytes(cur_ciphertext[pad_len:])
                    )
                ...
                if not faulty and len(candidate) != 1:
                    assert False, (
                        f"[ERROR] <forge_CBC_ciphertext_with_padding_oracle> Failed to forge block {i}, index {j}: unique candidate expected"
                    )
                for x in candidate:
                    q_next.append(
                        ciphertext[: pad_len * i + j]
                        + [x]
                        + ciphertext[pad_len * i + j + 1 :]
                    )
            q = q_next
        for k in range(pad_len):
            plaintext[pad_len * i + k] ^= pad_len
    res = [
        (bytes(ciphertext[:pad_len]), bytes(ciphertext[pad_len:])) for ciphertext in q
    ]
...
# if faulty, padding_oracle returns True on some instances where it should return False
# Returns a tuple of iv and ciphertext if not faulty, otherwise all possible such tuples
def forge_CBC_ciphertext_with_batched_padding_oracle(
    plaintext: bytes,
    padding_oracle_request,
    padding_oracle_read,
    batch_size=2**8,
    hint=None,
    faulty=False,
    ...
    for i in reversed(range(n // pad_len - int(hint != None))):
        for j in reversed(range(pad_len)):
            print(
                f"[INFO] <forge_CBC_ciphertext_with_batched_padding_oracle> Forging block {i} at index {j}"
            )
            for k in range(j, pad_len):
                plaintext[pad_len * i + k] ^= pad_len - j ^ (
                    pad_len - j - 1 if k > j else 0
            ...
            for ciphertext in q:
                print(
                    f"[INFO] <forge_CBC_ciphertext_with_batched_padding_oracle> {ciphertext = }"
                )
                candidate = []
                ...
                if not faulty and len(candidate) != 1:
                    assert False, (
                        f"[ERROR] <forge_CBC_ciphertext_with_batched_padding_oracle> Failed to forge block {i}, index {j}: unique candidate expected"
                    )
                for x in candidate:
                    q_next.append(
                        ciphertext[: pad_len * i + j]
...
def forge_CBC_plaintext_with_padding_oracle(
    iv: bytes, ciphertext: bytes, padding_oracle, faulty=False, pool=list(range(2**8))
):
    pool = list(sorted(set(map(int, pool))))
    assert 0 < len(pool) and 0 <= min(pool) and max(pool) < 2**8
    ...
    for i in reversed(range(n // pad_len)):
        for j in reversed(range(pad_len)):
            print(
                f"[INFO] <forge_CBC_plaintext_with_padding_oracle> Forging block {i} at index {j}"
            )
            for k in range(j, pad_len):
                ciphertext[pad_len * i + k] ^= pad_len - j ^ (
                    pad_len - j - 1 if k > j else 0
                )
            q_next = []
            for plaintext in q:
                print(
                    f"[INFO] <forge_CBC_plaintext_with_padding_oracle> {plaintext = }"
                )
                candidate = []
                for x in pool:
                ...
                if not faulty and len(candidate) != 1:
                    assert False, (
                        f"[ERROR] <forge_CBC_plaintext_with_padding_oracle> Failed to forge block {i}, index {j}: unique candidate expected"
                    )
                for x in candidate:
                    q_next.append(
                        plaintext[: pad_len * i + j]
                        + [x]
                        + plaintext[pad_len * i + j + 1 :]
                    )
            q = q_next
        for k in range(pad_len):
...
# if faulty, padding_oracle returns True on some instances where it should return False
# Returns the plaintext if not faulty, otherwise all possible such plaintexts, such that CBC(plaintext, iv) = ciphertext
def forge_CBC_plaintext_with_batched_padding_oracle(
    iv: bytes,
    ciphertext: bytes,
    padding_oracle_request,
    padding_oracle_read,
    ...
    for i in reversed(range(n // pad_len)):
        for j in reversed(range(pad_len)):
            print(
                f"[INFO] <forge_CBC_plaintext_with_batched_padding_oracle> Forging block {i} at index {j}"
            )
            for k in range(j, pad_len):
                ciphertext[pad_len * i + k] ^= pad_len - j ^ (
                    pad_len - j - 1 if k > j else 0
            ...
            for plaintext in q:
                print(
                    f"[INFO] <forge_CBC_plaintext_with_batched_padding_oracle> {plaintext = }"
                )
                candidate = []
                for batch_start in range(0, len(pool), batch_size):
                    for x in pool[batch_start : batch_start + batch_size]:
                ...
                if not faulty and len(candidate) != 1:
                    assert False, (
                        f"[ERROR] <forge_CBC_plaintext_with_batched_padding_oracle> Failed to forge block {i}, index {j}: unique candidate expected"
                    )
                for x in candidate:
                    q_next.append(
                        plaintext[: pad_len * i + j]
...
if __name__ == "__main__":
    ...
    def test_forge_CBC_ciphertext_with_padding_oracle():
        ...
        plaintext = pad(
            b"Test_text_123234234!!{testing_zzzzzzzzz}_1232342245234234243", 16
        )
        ...
        iv, ciphertext = forge_CBC_ciphertext_with_padding_oracle(
            plaintext, padding_oracle
        )
        assert ciphertext == AES.new(key, AES.MODE_CBC, iv).encrypt(plaintext)
        iv, ciphertext = forge_CBC_ciphertext_with_padding_oracle(
            plaintext, padding_oracle, (hm, hiv, hct)
        )
        assert ciphertext == AES.new(key, AES.MODE_CBC, iv).encrypt(plaintext)
    ...
    def test_forge_CBC_ciphertext_with_batched_padding_oracle():
        ...
        iv, ciphertext = forge_CBC_ciphertext_with_batched_padding_oracle(
            plaintext, padding_oracle_request, padding_oracle_read, batch_size=79
        )
        assert ciphertext == AES.new(key, AES.MODE_CBC, iv).encrypt(plaintext)
        iv, ciphertext = forge_CBC_ciphertext_with_batched_padding_oracle(
            plaintext,
    ...
    def test_forge_CBC_plaintext_with_padding_oracle():
        ...
        assert (
            forge_CBC_plaintext_with_padding_oracle(iv, ciphertext, padding_oracle)
            == plaintext
        )
    ...
    def test_forge_CBC_plaintext_with_batched_padding_oracle():
        ...
        assert (
            forge_CBC_plaintext_with_batched_padding_oracle(
                iv,
                ciphertext,
                padding_oracle_request,
                padding_oracle_read,

...
def recover_ECB_IGE_plaintext_with_padding_oracle(
    m0: bytes,
    c0: bytes,
    ciphertext: bytes,
    padding_oracle,
    execution_type: str = "naive",
):
        ...
        for j in reversed(range(pad_len)):
            if execution_type == "naive":
                ...
                for x in range(256):
                    cur[j] = x
                    if padding_oracle(
                        bytes(plaintext[pad_len * i : pad_len * (i + 1)]),
                        bytes(cur[:pad_len]),
                        bytes(cur[pad_len:]),
                    ):
                        if j == 0:
                            temp[j] = x
                            print(
                                f"[INFO] <recover_ECB_IGE_plaintext_with_padding_oracle>J Index {pad_len * i + j} matched with {x}"
                            )
                            break
                    ...
                    print(
                        f"[ERROR] <recover_ECB_IGE_plaintext_with_padding_oracle> Failed to recover byte at index {pad_len * i + j}"
                    )
                    assert False
                ...
                print(
                    f"[ERROR] <recover_ECB_IGE_plaintext_with_padding_oracle> Invalid execution type"
                )
                assert False
            for k in range(j, pad_len):
                temp[k] ^= pad_len - j ^ pad_len - j + 1
        for j in range(pad_len):
            plaintext[pad_len * (i + 1) + j] = (
                ciphertext[pad_len * i + j] ^ temp[j] ^ pad_len + 1
            )

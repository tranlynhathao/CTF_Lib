...
class twin_prime_generator:
    ...
    def _generate_candidates(self):
        ...
        self.candidates = [
            getrandbits(self.max_bit) for _ in range(self.candidate_count)
        ]
    ...
    def get_twin_prime(self, nbit: int):
        ...
        while True:
            for x in self.candidates:
                ...
                if (
                    (x - 1).bit_length() == nbit
                    and is_pseudoprime(x - 1)
                    and is_pseudoprime(x + 1)
                ):
                    if not is_prime(x - 1):
                        print(
                            f"[INFO] <twin_prime_generator> Found counter example for is_pseudoprime: {x - 1}"
                        )
                        exit(-1)
                    if not is_prime(x + 1):
                        print(
